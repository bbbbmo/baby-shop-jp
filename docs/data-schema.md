...
마이그레이션의 기존 사용자 백필은 **이메일 가입자만** 대상입니다. 소셜 가입은
그때까지 동의 절차가 없었으므로 동의한 적이 없고, 여기에 행을 넣으면 받지 않은
동의를 기록하는 동시에 `hasConsentRecord()`가 `true`가 되어 그들이 동의 화면을
영영 보지 못합니다. 레코드를 비워두면 다음 로그인 때 콜백이 동의 화면으로
