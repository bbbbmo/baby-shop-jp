...
| **2단계** | 가격 · 통화 · 배송비 | 🔄 **Task 6/10 진행 중** |
| **3단계** | 주소 폼 · 주소 검색 · 주문 | ⬜ 미착수 (설계만 있음) |
...
설계만 되어 있고 구현 계획은 없습니다.
[`docs/specs/2026-08-28-multi-market-orders-design.md`](./specs/2026-08-28-multi-market-orders-design.md)의 주소·주문 부분이 3단계 내용입니다.

들어갈 것

- `orders`에 마켓 컬럼, 후리가나를 nullable로
- `order_items`에 한국어 상품명
- 마켓별 주소 스키마 (일본은 후리가나 필수 · 7자리 우편번호, 한국은 후리가나 없음 · 5자리)
- 도로명주소 검색 (juso.go.kr) — **API 키 발급 필요**
- 체크아웃에서 주문 스냅샷에 두 언어 상품명 저장

**2단계를 머지한 뒤 계획을 짜는 게 정확합니다.** 파일 경로와 구조가 2단계 결과에 따라 달라집니다.
