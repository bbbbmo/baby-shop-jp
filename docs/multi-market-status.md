...
| 9 | 한국어 화면의 첫 응답 언어 표시 — 완료 (루트 레이아웃 분리) | — |
| 10 | 문서 갱신 | 소 |
...
- 팝업 안에서 입력한 **상세주소(동·호수)도 함께 넘어옵니다**. 주문서의 「상세주소」 칸에 들어가며,
  팝업에서 비워 두고 오면 주문서에 이미 적어 둔 값을 덮어쓰지 않습니다
- **한국은 상세주소가 필수입니다** (아파트·오피스텔 비중). 일본은 「建物名（任意）」 그대로 선택입니다.
  검증은 `checkoutSchema(market)` 한 곳에 있고 화면과 `/api/checkout`이 같은 스키마를 씁니다
