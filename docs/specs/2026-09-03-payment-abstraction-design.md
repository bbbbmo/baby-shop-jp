...
**결제가 나중에 확정되는 수단은 이 구조로 안 된다.** 가상계좌·무통장입금, 일본 편의점 결제와
Pay-easy는 손님이 복귀한 시점에 아직 입금이 안 되어 있고 며칠 뒤 webhook으로 통보된다. 붙이려면
`PaymentProvider`에 네 번째 메서드와 webhook 라우트, 「입금 대기」 상태가 필요하다 —
`types.ts`가 깨진다. **첫 PG를 카드 위주로 시작한다는 전제**에서만 아래 「계약 후 남는 일」이
한 파일로 끝난다. 일본 마켓을 열 때 이 항목이 먼저 온다.
